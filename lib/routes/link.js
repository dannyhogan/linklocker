const { Router } = require('express');
const ensureAuth = require('../middleware/ensureAuth');
const Link = require('../models/Link');
const User = require('../models/User');

module.exports = Router()
  .post('/', ensureAuth, (req, res, next) => {
    const owner = req.user._id;
    const { url, title, isLocked, password } = req.body;
    Link
      .create({ owner, url, title, isLocked, password: isLocked && password })
      .then(link => {
        res.send(link);
      })
      .catch(next);
  })

  .post('/:id', (req, res, next) => {
    const { id } = req.params;

    Link
      .findByIdAndUpdate(id, { $inc: { clickCount: 1 } })
      .then(() => {
        res.send(200);
      })
      .catch(next);
  })

  .get('/:username', ensureAuth, async (req, res, next) => {
    const { username } = req.params;

    const selfProfile = username === req.user.username;

    let profile;

    if(!selfProfile) {
      profile = await User.findOneAndUpdate({ username }, { $inc: { profileViews: 1 } }, { new: true });
    } else {
      profile = await User.findOne({ username });
    }


    if(!profile) {

      return res.status(400).send({
        message: 'Unable to find profile!'
      });

    } else {

      Link
        .find({ owner: profile })
        .then(links => res.send(links))
        .catch(next);
    }
  });
